...
#### CLASS METHODS
      ...
      def self.parse_twitter_handle(url)
         if handle = url.strip.match(URL_REGEXES[:twitter_profile])
            return handle[1]
         end
      end
      ...
      def self.detect_twitter_accounts(parsed_content)
         ...
         twitter_links = parsed_content.css('a').select{|a| a['href'] =~ URL_REGEXES[:twitter_profile]}

         # look for link text with "follow" or "me"
         _follow_text = URL_REGEXES[:follow_text]
